...
#define GLGE_DEFAULT_3D_FRAGMENT std::string("#version 300 es\nprecision highp float;layout(location = 0) out vec4 Albedo;layout(location = 1) out vec4 Normal;layout(location = 2) out vec4 Position;layout(location = 3) out vec4 Roughness;in vec4 color;in vec2 texCoord;in vec3 normal;in vec3 currentPos;float emmission = 0.2f;uniform vec4 objCol;uniform float roughness;uniform float metalic;uniform sampler2D AmbientMap;uniform sampler2D NormalMap;uniform sampler2D RoughnessMap;uniform sampler2D Texture;uniform samplerCube shadowMap;float exposure = 0.1;uniform int usedTextures;uniform bool NormalMapIsActive;uniform bool RoughnessMapIsActive;uniform vec3 cameraPos;uniform float farPlane;float rough;vec3 actualNormal = vec3(0,0,0);void main()\n{vec4 col = objCol + color + texture(Texture, texCoord);if (usedTextures > 1){col += vec4(texture(AmbientMap, texCoord).rgb, col.w);}if (RoughnessMapIsActive){rough = texture(RoughnessMap, texCoord).r;}else{rough = roughness;}vec3 n = normalize(normal);if (NormalMapIsActive){n = normalize(normal + (texture(NormalMap, texCoord).rgb * 2.f - 1.f));}actualNormal = n;vec4 FragColor = vec4(0,0,0,0);Albedo = min(max(col, vec4(0,0,0,0)), vec4(1,1,1,1));Normal = vec4((actualNormal + vec3(1,1,1)) / vec3(2,2,2),1);Position = vec4(currentPos,1);Roughness = vec4(rough,metalic,0,1);gl_FragDepth = gl_FragCoord.z;}")
