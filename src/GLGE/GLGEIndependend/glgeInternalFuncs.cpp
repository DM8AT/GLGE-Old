...
"    vec4 fpinl = vec4(p.pos,1) * mat4(l.lightSpaceMatR0, l.lightSpaceMatR1, l.lightSpaceMatR2, l.lightSpaceMatR3);",
"    vec3 proj = (fpinl.xyz / fpinl.w) * 0.5 + 0.5;",
"    float bias = max(0.025f * (1.f - dot(p.normal, normalize(l.pos))), 0.005f);",
"    float shadowDepth = (texture(sampler2D(l.shadowMap), proj.xy).r) - bias;",
"    return (shadowDepth > proj.z) ? 0.f : 1.f;",
"}",
"",
