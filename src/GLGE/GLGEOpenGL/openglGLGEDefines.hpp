...
#define GLGE_DEFAULT_TRANSPARENT_SHADER std::string("#version 450 core\n#extension GL_ARB_bindless_texture : require\n#define PI 3.14159265\nprecision highp float;layout(location = 5) out vec4 Max;layout(location = 7) out vec4 Col;layout(location = 3) out vec4 rml;uniform sampler2D glgeLitMap;in vec4 color;in vec2 texCoord;in vec3 normal;in vec3 currentPos;\n#include <glgeObject>\n#include <glgeCamera>\n#include <glgeMaterial>\nuniform bool glgePass;float rough = 0.f;int iteration = 0;vec3 actualNormal = vec3(0,0,0);float lightRadius = 0.1;float a = 0.f;float b = 0.f;vec3 pos;float metallic = 0.f;#include <glgeLightEvaluationFunction>\n#include <glgeLights>\nvoid main(){vec4 col = glgeColor + color;col.rgb *= (1-int(glgeAmbientMapActive));col.rgb += texture(sampler2D(glgeAmbientMap), texCoord).rgb * int(glgeAmbientMapActive);if(col.w == 0.f){discard;}if (!glgePass && col.w!=1.f){discard;}if (glgePass && col.w==1.f){discard;}float rough = texture(sampler2D(glgeRoughnessMap), texCoord).r * float(int(glgeRoughnessMapActive));rough += glgeRoughness * (1.f - float(int(glgeRoughnessMapActive)));actualNormal = normal;pos = currentPos;vec4 finColor = col;if (bool(glgeLit)){finColor.rgb = vec3(0);glgePixel d = glgePixel(col.rgb,currentPos,normal,rough, metallic);for (int i = 0; i < glgeActiveLights; i++){vec3 light = glgeEvaluateLight(glgeLights[i], d);finColor.xyz += light;}}Col = finColor;Max = vec4(0,0,1,1);rml.rgb = vec3(rough, glgeMetalic, float(glgeLit));}")
